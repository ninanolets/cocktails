  ...
  likeButtons.forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      const likeCountElement = document.querySelector(
        `#commentlike-${commentId} .like-count`
      );
...
const addPostLikes = () => {
  ...
  if (likeButton) {
    likeButton.addEventListener("click", async () => {
      ...
      const res = await fetch(reqUrl);
      const data = await res.json();
      const { isPostLiked } = data;
      console.log(data);

      const likePostCountElement = document.querySelector(
        `#postlike-${postId} .like-post-count`
      );
      const likePostImage = document.querySelector(`#postlike-${postId} img`);

      if (!likePostCountElement) return;
      const likePostCount = Number(likePostCountElement.innerText);

      if (isPostLiked) {
        likePostImage.classList.add("bg-likes-after-post");
        likePostCountElement.innerText = likePostCount + 1;
      } else {
        likePostImage.classList.remove("bg-likes-after-post");
        likePostCountElement.innerText = likePostCount - 1;
      }
    });
  }
